...
public class NoHPAxisEver extends HBRHitAndTypeAttackCard {
    public static final String ID = makeID(NoHPAxisEver.class.getSimpleName()); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        for (AbstractMonster monster : (AbstractDungeon.getMonsters()).monsters) {
            if (!monster.isDead && !monster.isDying) {
                addToBot(new GainBlockAction(monster, monster, this.block));
            }
        }
    }
